...
输出严格 JSON 格式:
{
  ...
  "ozon_category_suggestion": "建议的Ozon类目路径",
  "attributes": {
    "brand": "",
    "model": "",
    "color": "",
...
async function main() {
  ...
  for (const s of slugs) {
    ...
    if (!product) { console.log(`  跳过 ${s}: 无 product.json`); continue; }

    console.log(`  推理: ${s}`);
    try {
      const inferred = await inferAttributes(product);
      await writeJson(path.join(productsDir, s, "inferred.json"), {
        ...inferred,
        inferred_at: new Date().toISOString(),
        source_slug: s,
      });
      console.log(`    ✓ ${inferred.title_ru?.slice(0, 50) || "(无标题)"}`);
      success++;
    } catch (err) {
  ...
  console.log(`\n[Stage 4] 完成: ${success}/${slugs.length} 成功`);
}
